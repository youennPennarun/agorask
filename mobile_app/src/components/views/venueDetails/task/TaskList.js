...
class TaskList extends Component {
  ...
  componentWillReceiveProps(next: TaskListPropsType) {
    if (this.props.tasks !== next.tasks) {
      this.setState(state => ({
  ...
  buildAnimations(currentTasks: Array<TaskType>, nextTasks: Array<TaskType>, animationState: Object): TaskListStateType {
    ...
    currentTasks.forEach(task => {
      if (!nextTasks.find(t => t._id === task._id)) {
        this.pendingExitAnimations.push(Animated.timing(nextAnimationState[task._id], {
          toValue: -width,
        }));
      }
      tasks.push(task);
    ...
    nextTasks.forEach(task => {
      if (!currentTasks.find(t => t._id === task._id)) {
        ...
        this.pendingEnterAnimations.push(Animated.timing(nextAnimationState[task._id], {
          toValue: 0,
        }));
        tasks.push(task);
      }
